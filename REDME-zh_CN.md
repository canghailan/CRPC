# CRPC (Concise RPC) 协议规范与技术白皮书：构建易用、高性能 RPC 框架
...
构建了一个标准化、支持平滑演进的通信框架，实现“简明易用，极致性能”的设计愿景。
...
## 大数据与 AI 扩展：Arrow 支持
为了实现类似 ADBC (Arrow Database Connectivity) 的功能，CRPC 允许直接返回 Arrow 格式：
- **查询结果透明化**：客户端通过 Accept: application/vnd.apache.arrow.stream 发起查询，服务端将数据库结果以 RecordBatches 形式流式回传，跳过 JSON/CBOR 的行列转换损耗
- **AI 张量传输**：在模型推理中，Payload 直接承载 Arrow 封装的张量数据，利用多维数组支持和 Zstd 字典压缩，性能可比常规序列化提升 10 倍以上
